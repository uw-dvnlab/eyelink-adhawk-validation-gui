...
for n=1:13
    ...
    for trial=1:length(dataEL)
        ...
        for stream=1:2 % 1: Right, 2: Left
            ...
            [cEL,lagsEL] = xcorr(dvaTAR(1:dataLen),dvaEL(1:dataLen),FPS/5,'normalized');
            [cAH,lagsAH] = xcorr(dvaTAR(1:dataLen),dvaAH(1:dataLen),FPS/5,'normalized');
            ...
            % Remove outliers
            [timeEL, dvaEL] = removeOutliers(timeEL, dvaEL);
            [timeAH, dvaAH] = removeOutliers(timeAH, dvaAH);
...
% Export table
writetable(table_out, './export/gains_hsp.csv')